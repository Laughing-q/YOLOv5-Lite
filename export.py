...
from models.common import Conv, CBH, Dense, LC_SEModule, RepVGGBlock
    ...
    for k, m in model.named_modules():
        if isinstance(m, Conv):  # assign export-friendly activations
            ...
            if isinstance(m.act, nn.Hardswish):
                m.act = Hardswish()
        if isinstance(m, (CBH, Dense)):  # assign export-friendly activations
            if isinstance(m.hardswish, nn.Hardswish):
                m.hardswish = Hardswish()
        if isinstance(m, LC_SEModule):
            m.SiLU = SiLU()
